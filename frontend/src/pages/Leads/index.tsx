...
import { getLeads, createLead, LeadFromAPI } from '../../services/leads';
...
const MOCK_DATA: KanbanCol[] = [
  {
    id: 'novo_lead', title: 'Novo Lead', totalValue: 120000, headerColor: '#c53030',
    leads: [
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {lead.statusUpdatedAt && <span style={{ fontSize: 10, color: '#a0aec0' }}>{lead.statusUpdatedAt}</span>}
      </div>
    </div>
  );
}
...
function CreateLeadModal({ onClose, onSuccess }: CreateLeadModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (!form.origin.trim()) { setError('Origem é obrigatória'); return; }
  ...
  return (
    ...
    >
      <div style={{ background: '#fff', borderRadius: 12, padding: 28, width: 440, maxWidth: '90vw', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        {/* Header */}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {error && (
            <div style={{ background: '#fff5f5', border: '1px solid #fed7d7', borderRadius: 6, padding: '10px 12px', fontSize: 13, color: '#c53030' }}>
              {error}
            </div>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
          <button
            onClick={onClose}
            style={{ flex: 1, padding: '10px 0', border: '1px solid #e2e8f0', borderRadius: 6, background: '#fff', color: '#4a5568', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
            Cancelar
          </button>
          <button
            ...
            disabled={loading}
            style={{ flex: 2, padding: '10px 0', border: 'none', borderRadius: 6, background: loading ? '#9ae6b4' : '#38a169', color: '#fff', fontSize: 13, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', transition: 'background 0.15s' }}
          >
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      if (leads && leads.length > 0) {
        ...
        setColumns(groupLeadsIntoColumns([]));
        setUsingMock(false);
      }
    } catch (err) {
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', background: '#fff', borderBottom: '1px solid #e2e8f0', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#38a169', color: '#fff', border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <path d="M7 1v12M1 7h12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
          ...
          {usingMock && (
            <span style={{ fontSize: 11, color: '#dd6b20', background: '#fffaf0', border: '1px solid #f6ad55', borderRadius: 4, padding: '3px 8px' }}>
              ⚠ Dados mock — API offline
            </span>
          )}
